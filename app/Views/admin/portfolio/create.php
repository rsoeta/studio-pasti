...
<div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 max-w-2xl">
    <form action="<?= base_url('admin/portfolio/store') ?>" method="POST" class="space-y-5">
        <?= csrf_field() ?>

        <div>
            <label class="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-1">Nama / Judul Aplikasi</label>
            <input type="text" name="judul" required placeholder="Contoh: Implementasi OpenSID Premium" class="w-full bg-slate-50 border border-slate-200 text-slate-800 text-sm rounded-lg p-2.5 outline-none focus:border-blue-500 transition">
        </div>

        <div>
            <label class="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-1">Deskripsi Sistem</label>
            <textarea name="deskripsi" rows="3" required placeholder="Jelaskan fitur utama atau fungsi ringkas aplikasi..." class="w-full bg-slate-50 border border-slate-200 text-slate-800 text-sm rounded-lg p-2.5 outline-none focus:border-blue-500 transition"></textarea>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
                <label class="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-1">Tautan GitHub</label>
                <input type="text" name="link_github" placeholder="https://github.com/..." class="w-full bg-slate-50 border border-slate-200 text-slate-800 text-sm rounded-lg p-2.5 outline-none focus:border-blue-500 transition">
            </div>
            <div>
                <label class="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-1">URL Aplikasi Demo</label>
                <input type="text" name="link_demo" placeholder="https://demo.studio-pasti.net/..." class="w-full bg-slate-50 border border-slate-200 text-slate-800 text-sm rounded-lg p-2.5 outline-none focus:border-blue-500 transition">
            </div>
            <div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <input type="text" name="ikon" placeholder="fa-server" class="w-full bg-slate-50 border border-slate-200 text-slate-800 text-sm rounded-lg p-2.5 outline-none focus:border-blue-500 transition">
            </div>
            <div>
        ...
        <button type="submit" class="bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold px-5 py-3 rounded-lg transition shadow-md flex items-center">
            <i class="fa-solid fa-floppy-disk mr-1.5"></i> Simpan Portofolio
        </button>
    </form>
</div>
